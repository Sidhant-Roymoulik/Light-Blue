...
// Upgrades over Version 2.1:
// 		Run MTD(f) and MTD(bi) concurrently

type engine_version_2_2 struct {
	EngineClass
}

func new_engine_version_2_2() engine_version_2_2 {
	return engine_version_2_2{
		EngineClass{
			name:       "Version 2.2 (MTD Selection)",
			max_ply:    0,
...
func (engine *engine_version_2_2) run(position *chess.Position) (best_eval int, best_move *chess.Move) {
	resetCounters()
...
func (engine *engine_version_2_2) iterative_deepening(position *chess.Position) (best_eval int, best_move *chess.Move) {
...
func (engine *engine_version_2_2) mtd_f(position *chess.Position, g int, eval_chan chan int, move_chan chan *chess.Move) {
	mtd_f_iter := 0
	eval := g
...
func (engine *engine_version_2_2) mtd_bi(position *chess.Position, eval_chan chan int, move_chan chan *chess.Move) {
	mtd_bi_iter := 0
	eval := 0
	upper := CHECKMATE_VALUE
	lower := -CHECKMATE_VALUE
...
func (engine *engine_version_2_2) minimax_start(position *chess.Position, turn bool, alpha int, beta int, hash_history []uint64) (eval int, move *chess.Move) {
	states++
...
func (engine *engine_version_2_2) minimax(position *chess.Position, ply int, turn bool, alpha int, beta int, hash_history []uint64) (eval int) {
	states++
...
func (engine *engine_version_2_2) q_search(position *chess.Position, ply int, turn bool, alpha int, beta int) (eval int) {
	q_states++
...
func (engine *engine_version_2_2) Is_Draw_By_Repetition_Local(hash uint64, move_history []uint64) bool {
