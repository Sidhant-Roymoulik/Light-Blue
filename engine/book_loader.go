...
// as well as a polyglot hash generator.
...
func GenPolyglotHash(pos *chess.Position) (hash uint64) {
	for sq, piece := range pos.Board().SquareMap() {
		if piece.Type() != chess.NoPieceType {
			hash ^= Random64[(uint16(piece.Type())*2+uint16(piece.Color()^1))*64+uint16(sq)]
		}
	}

	if pos.EnPassantSquare() != chess.NoSquare {
		hash ^= Random64[En_Passant+int(uint8(pos.EnPassantSquare().File()))]
	}

	if pos.CastleRights().CanCastle(chess.White, chess.KingSide) {
		hash ^= Random64[CastleWKSHash]
	}

	if pos.CastleRights().CanCastle(chess.White, chess.QueenSide) {
		hash ^= Random64[CastleWQSHash]
	}
	if pos.CastleRights().CanCastle(chess.Black, chess.KingSide) {
		hash ^= Random64[CastleBKSHash]
	}
	if pos.CastleRights().CanCastle(chess.Black, chess.QueenSide) {
