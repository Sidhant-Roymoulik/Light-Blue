...
type move_engine struct{}

func (move_engine) CalcMoves(pos *Position, first bool) []*Move {
...
func (move_engine) Status(pos *Position) Method {
	hasMove := false
	if pos.validMoves != nil {
		hasMove = len(pos.validMoves) > 0
	} else {
		hasMove = len(move_engine{}.CalcMoves(pos, true)) > 0
	}
